LogisticRMoE = function(Xmat, Ymat, K, R, Lambda, Gamma, option)
{
...
L2 = LOG(X, Y, wk, eta, lambda, gamma, rho)
step = 1
arr[step] = L2
...
repeat
{
  ...
  L1 = L2
  #---------E-step
  tau = e.step(eta, wk, Y, X, K, R)
  #---------M-step
  wk = CoorGateP(X, wk, tau, gamma, rho)
  eta = pm.step(X, Y, U, R, eta, tau, lambda, cl, option)
  L2 = LOG(X, Y, wk, eta, lambda, gamma, rho)
